...
let parseFile fichier : fnc =
	...
	let clauseVideVue = ref false in
	...
	let rec aux fichier sortie =
		...
		(
			...
			(
				...
				(
					...
					aux fichier ((clause,!nbClausesLues)::sortie);
				)
				
			)
			else if (Str.string_match (Str.regexp " *0") ligne 0) then
			(
				if (!headerVu = false) then failwith ""
				else
				(
				clauseVideVue := true;
				aux fichier (sortie);
		...
		with End_of_file ->
			(
				...
				if( !nbClausesLues = 0) then
				(
					if (!clauseVideVue = true) then [([],0)] else []
				)
				else
				(
					sortie
			...
			|_ -> failwith "Format du fichier incorrect."
