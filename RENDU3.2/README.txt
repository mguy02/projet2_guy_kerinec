...
Nous avons encore une fois séparé en plusieurs parties, et séparé le travail:
	...
	3) Graphe des conflits
		Le fichier .dot est généré dans le fichier output.dot.
		Il faut alors executer la commande 'dot -Tpdf output.dot -o output.pdf' pour générer le pdf correspondant.
		...
		b) Idée pour construire le graphe
			...
			La construction se fait en plusieurs étapes: tout d'abord, on parcours l'ancienne fnc pour trouver le conflit. Ensuite, on va créer une liaison entre leurs parents (litteraux qui les impliquent) et eux.
			Enfin, on trouve le noeud uip (en construisant au passage l'ensemble UIP) en remontant depuis les noeuds conflits. On peut remonter facilement étant donné que l'on a la liste des parents. Pour partir du bas, il suffit de renverser la liste des valuations.
		
		c) Savoir quelle clause apprendre suivant l'ensemble UIP
			C'est tout simplement les noeuds parents de noeuds de l'ensemble UIP n'étant pas du niveau du conflit + le noeud UIP.
			Sur le graphe, cela se voit comme les noeuds blancs (et le jaune) impliquant des noeuds violets.
			On détermine le niveau du backtrack comme étant le niveau le plus élevé de ces "noeuds parents" (sans prendre en compte le noeud UIP). On adapte donc le backtrack initial (dans dpll.ml) et ajoute la nouvelle clause.
		
	Le travail a été séparé, mais globalement a été traité de maniere égale.
...
Divers:
	...
	- Nous avons fait un générateur de tests (de fichier .cnf et de fichier .for). C'est le fichier 'generate.py', fait en Python3, qui s'excute de la manière suivante : 'python3 generate.py'.
	  Ce générateur utilise un fichier, 'numeroTest', pour ne pas s'occuper du nom. Les fichiers sont générés dans tests/generated/ si .cnf et tests/generated/tseitin si .for. A noter toutefois qu'il n'est pas très efficace car les litteraux sont choisis uniformément. Mais cela a quand même l'interêt de pouvoir tester sur des gros tests.
