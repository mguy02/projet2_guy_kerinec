...
def generateClause (indmax, nbClause):
	global l
	nbLitt = random.randint(1, 2*indmax)
	output = ""
...
if choix == 1:
	...
	for i in range(nbClause):
		output += generateClause(indmax, nbClause)
	
	
	#Puis on ecrit output dans un fichier
	nomFichier = "tests/generated_"+str(numTestCNF)+".cnf"
	...
	nomFichier = "tests/tseitin/generated_"+str(numTestFOR)+".for"
	fichier = open(nomFichier, "w")
