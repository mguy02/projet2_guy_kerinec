...
(* TODO:
REFLECHIR : Deduit de 1true/1false ou uniquement déduit de 1 ?
*)
...
let getDeduction (litt:litteral) (numClause:int) (fnc:fnc) (valuation:affectations) : litteral list =
	...
	|t::q -> if (t = litt) then parcours_clause q lr(* on saute le litt *)
		...
		(
			if (est_affecte (abs(t)) valuation) then ((*print_string (soi(abs(t))^" est affecte\n");*) parcours_clause q (abs(t)::lr))
			else ((*print_string (soi(abs(t))^" est pas affecte\n");*) parcours_clause q lr)	(* on ajoute pas *)
		)
	in
